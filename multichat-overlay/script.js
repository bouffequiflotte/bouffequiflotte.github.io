...
async function TwitchCheer(data) {
	...
	titleDiv.innerText = `${username} jette ${bits} bits dans la marmite ! ✨`;
...
async function TwitchAnnouncement(data) {
	...
	// Set the card header
	iconDiv.innerText = "📣";
	titleDiv.innerText = "Oyez, oyez, mes flotteurs !";
...
async function TwitchFollow(data) {
	...
	titleDiv.innerText = `${username} rejoint le radeau ! Bienvenue à bord 🌊`;
...
async function TwitchSub(data) {
	...
	if (!isPrime)
		titleDiv.innerText = `${username} embarque en Tier ${subTier.charAt(0)} ! ⚓`;
	else
		titleDiv.innerText = `${username} monte à bord avec l’argent de Jeff ! 🛳️`;
...
async function TwitchResub(data) {
	...
	if (!isPrime)
		titleDiv.innerText = `🌊 ${username} rembarque en Tier ${subTier.charAt(0)} — ${cumulativeMonths} mois à flotter avec nous !`;
	else
		titleDiv.innerText = `${username} rembarque avec son Prime pour le ${cumulativeMonths}e mois ! ⛵`;
...
async function TwitchGiftSub(data) {
	...
	titleDiv.innerText = `${username} offre une place à bord en Tier ${subTier.charAt(0)} à ${recipient} ! 🎁`;
	if (cumlativeTotal > 0)
		contentDiv.innerText = `⚓ ${cumlativeTotal} places offertes au total ! Le radeau coule sous ta générosité ! 🌊`;
...
async function TwitchRewardRedemption(data) {
	...
	titleDiv.innerHTML = `🪙 ${username} troque ${channelPointIcon} ${cost} contre « ${rewardName} » ! Bien joué moussaillon`;
...
async function TwitchCustomPowerUpRedemption(data) {
	...
	titleDiv.innerHTML = `${username} utilise ${bitIcon} ${cost} pour déclencher « ${powerUpName} » ! ⚡`;
	contentDiv.innerText = `${userInput}`;
...
async function TwitchRaid(data) {
	...
	titleDiv.innerText = `${username} accoste sur le radeau`;
	contentDiv.innerText = `avec son équipage de ${viewers} flotteurs ! 🌊`;
...
async function TwitchWatchStreak(data) {
	...
	titleDiv.innerText = `${displayName} en est à sa ${watchStreak}e traversée consécutive ! 🔥`;
...
function YouTubeSuperChat(data) {
	...
	// Set message text
	titleDiv.innerText = `🪙 ${data.user.name} envoie un Super Chat de ${data.amount} pour soutenir le radeau !`;
	if (data.message)
...
function YouTubeSuperSticker(data) {
	...
	titleDiv.innerHTML = `✨ ${user} colle un Super Sticker de ${amount} sur la coque ! 🛶`;
...
function YouTubeNewSponsor(data) {
	...
	// Set message text
	titleDiv.innerText = `⭐ Nouveau cap franchi : ${data.levelName} ! Le radeau prend le large 🧭`;
	contentDiv.innerText = `Bienvenue à bord, ${data.user.name} ! 🌊`;
...
function YouTubeGiftMembershipReceived(data) {
	...
	// Set message text
	titleDiv.innerText = `🎁 ${data.gifter.name} offre une place à bord`;
...
function TipeeeStreamDonation(data) {
	...
	if (currency == "USD")
		titleDiv.innerHTML = `${tipeeeStreamIcon} ${user} met $${amount} dans la caisse de bord ! Merci 💛`;
	else
		titleDiv.innerHTML = `${tipeeeStreamIcon} ${user} met ${currency} ${amount} dans la caisse de bord ! Merci 💛`;
